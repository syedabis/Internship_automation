...
/**
 * Dispatch Email with Attachment via SendGrid
 */
async function sendInternshipEmail({ studentName, programName, recipientEmail, buffer, certNo, dateStr }) {
  const apiKey = process.env.SENDGRID_API_KEY;
  const fromEmail = process.env.EMAIL_USER || 'workshopcertificate@datacrumbs.org';
  ...
  console.log(`[OK] Email sent to ${recipientEmail}`);
}
