...
/**
 * Dispatch Email with Attachment via Resend (or SendGrid fallback)
 */
async function sendInternshipEmail({ studentName, programName, recipientEmail, buffer, certNo, dateStr }) {
  ...
  // Option 1: Use Resend if API Key is present
  if (resendApiKey) {
  ...
  // Option 2: Fallback to SendGrid if API Key is present
  if (sendgridApiKey) {
    sgMail.setApiKey(sendgridApiKey);
    const msg = {
      to: recipientEmail,
      from: fromEmail.includes('<') ? fromEmail.split('<')[1].replace('>', '').trim() : fromEmail,
      subject: `Internship Completion Certificate - ${studentName} (${programName})`,
      html: htmlContent,
      attachments: [
        {
    ...
    console.log(`[OK] SendGrid email sent to ${recipientEmail}`);
    return;
  }

  throw new Error('Neither RESEND_API_KEY nor SENDGRID_API_KEY is configured in environment variables.');
}
